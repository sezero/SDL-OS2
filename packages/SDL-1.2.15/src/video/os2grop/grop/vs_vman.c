                     ...
                     PRECTL prectl)
{
  ...
  else
  {
    ...
    HPS      hps;
    HRGN     hrgnVisible, hrgnDest, hrgnUpdate;
    RGNRECT  rgnCtl;
    RECTL    rectlDef;
    ...
    // Make region to update.
    hrgnUpdate = GpiCreateRegion( hps, 0, NULL );
    // Make destination region.
    hrgnDest = GpiCreateRegion( hps, cRect, prectlDst );
    // Query visible region of the window.
    hrgnVisible = GpiCreateRegion( hps, 0, NULL );
    WinQueryVisibleRegion( pGrop->hwnd, hrgnVisible );

    // "AND" on visible and destination regions, result is update region.
    GpiCombineRegion( hps, hrgnUpdate, hrgnDest, hrgnVisible, CRGN_AND );
    GpiDestroyRegion( hps, hrgnDest );
    ...
    pptlSrcOrg = debugMAlloc( cRect * sizeof(POINTL) );
    if ( pptlSrcOrg == NULL )
    {
      debug( "Not enough memory" );
      debugFree( prectlDst );
    ...
    {
      debug( "Not enough memory" );
      debugFree( pptlSrcOrg );
      debugFree( prectlDst );
      return FALSE;
    }

    prectlScan = prectlDst;
    for( ulIdx = 0; ulIdx < cRect; ulIdx++, prectlScan++ )
    {
      pptlSrcOrg[ulIdx].x = prectlScan->xLeft - pGrop->rectlViewArea.xLeft;
      pptlSrcOrg[ulIdx].y = bmiSrc.ulHeight -
