...
{
    PBUFPSZ pBufPSZ = (PBUFPSZ) malloc(sizeof(BUFPSZ) + cbBuf);
...
int debug_counter(char *pcName, int iDelta)
{
    ...
    if (pScan == NULL) {
        pScan = (PDBGCOUNTER) calloc(1, sizeof(DBGCOUNTER));
        if (pScan == NULL)
...
void *debug_malloc(size_t size, char *pcFile, int iLine)
{
    ...
    if (size == 0) {
        debug_write("%s#%u : Zero bytes for memory block requested\n", pcFile, iLine);
        return NULL;
    }

    pBlock = malloc(size + sizeof(size_t));
    if (pBlock == NULL) {
...
char *debug_strdup(const char *src, char *pcFile, int iLine)
{
    ...
    dst = (char *) debug_malloc(strlen(src) + 1, pcFile, iLine);
    if (dst != NULL) {
        strcpy(dst, src);
    }
