...
#define _STR_LTrim(s)                               \
do {                                                \
    while (SDL_isspace(*(s)) || *(s)==',')          \
        s++;                                        \
} while(0)

#define _STR_RTrim(s, e)                            \
do {                                                \
    while ((e) > (s) && SDL_isspace(*((e) - 1)))    \
        (e)--;                                      \
} while(0)

#define _STR_Trim(s, e)                             \
do {                                                \
    _STR_LTrim((s), (e));                           \
    _STR_RTrim((s), (e));                           \
} while(0)

#define _STR_skipSeparators(ps, cs)                 \
do {                                                \
    while (*(ps) == cs || SDL_isspace(*(ps)))       \
        (ps)++;                                     \
} while(0)

#define _STR_skipNotSeparator(ps, cs)               \
do {                                                \
    while (*(ps) != '\0' && *(ps) != (cs))          \
        ps++;                                       \
} while(0)

#define _STR_skipNotSpace(ps)                       \
do {                                                \
    while (*(ps) != '\0' && !SDL_isspace(*(ps)))    \
        ps++;                                       \
} while(0)
